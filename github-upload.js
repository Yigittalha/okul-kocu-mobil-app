...
async function uploadToGithub() {
  try {
    ...
    rl.question('Commit mesajı: ', (message) => {
      ...
      const commitResult = runGitCommand(`git commit -m "${message || 'Automatic commit'}"`);
      console.log(commitResult);

      // Remote repo kontrol et
      const remotes = runGitCommand('git remote -v');
      if (!remotes.includes('origin')) {
        rl.question('Remote repository URL (örn: https://github.com/username/repo.git): ', (repoUrl) => {
