  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        Alert.alert('Giriş başarısız', 'Bilgiler boş veya yanlış gönderilmiştir');
      } else if (err.response?.data === false) {
        Alert.alert('Giriş başarısız', 'Kullanıcı adı veya şifre yanlış');
      } else {
        Alert.alert('Giriş başarısız', err.response?.data?.message || 'Bilinmeyen hata');
      }
    } finally {
      setLoading(false);
    }
        ...
        <View style={styles.formContainer}>
          ...
          {schoolCode && (
            <View style={[styles.schoolBadge, { backgroundColor: theme.accent }]}>
              <Text style={[styles.schoolInfo, { color: theme.primary }]}>📚 {schoolCode}</Text>
            </View>
          )}
          
          <View style={styles.inputContainer}>
            <Text style={[styles.inputLabel, { color: theme.text }]}>E-posta</Text>
            <TextInput
          ...
          <TouchableOpacity 
            style={[styles.button, { opacity: loading ? 0.6 : 1, backgroundColor: theme.accent }]}
            onPress={handleLogin} 
            disabled={loading}
          >
